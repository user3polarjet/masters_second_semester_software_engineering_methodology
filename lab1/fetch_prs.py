...
async def main():
    ...
    for pr_batch in itertools.batched(pr_rebuilds, 100):
        async with asyncio.TaskGroup() as tg:
            async def tg_task(number: int, pr_path: pathlib.Path):
                ...
                cmd = ("gh", "pr", "view", str(number), "--json", "body,number,title,author,additions,deletions,changedFiles,createdAt,mergedAt,closedAt,reviewDecision,comments,commits,reviews,labels")
                print(f'{cmd=}')
    ...
    if True:
        ser_prs_list: list[pr_classes.PullRequest] = []
        for pr in pr_paths:
            with open(pr) as fileio:
                data = json.load(fileio)
            ser_prs_list.append(pr_classes.PullRequest.deserialize(data))
        ser_prs = pr_classes.PrBlob(tuple(ser_prs_list))
    ...
    with open(BUILD_DIR / 'ser_prs.pickle', 'rb') as fileio:
        ...
        assert isinstance(ser_prs, pr_classes.PrBlob)
